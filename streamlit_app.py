...
with tab5:
    ...
    # Гистограмма цен для выбранного класса
    ax.hist(class_data, bins=30, color='#9b59b6', edgecolor='black', alpha=0.7)
    ax.set_xlabel('Цена билета')
    ax.set_ylabel('Количество')
    ax.set_title(f'Распределение цен билетов для {selected_class} класса')
    ax.axvline(class_data.mean(), color='red', linestyle='--', 
               label=f'Средняя цена: {class_data.mean():.2f}')
    ax.legend()
    ...
    with col3:
        st.metric("Медианная цена", f"{class_data.median():.2f}")
...
if st.checkbox("Показать подробную статистику"):
    st.subheader("Статистика по выживанию")
    
    # Статистика по разным категориям
    stats = pd.DataFrame({
        'Категория': ['Всего', 'Мужчины', 'Женщины', '1 класс', '2 класс', '3 класс'],
        'Всего': [
            len(df),
            len(df[df['Sex'] == 'male']),
        ...
        'Процент выживания': [
            f"{len(df[df['Survived'] == 1])/len(df)*100:.1f}%",
            f"{len(df[(df['Sex'] == 'male') & (df['Survived'] == 1)])/len(df[df['Sex'] == 'male'])*100:.1f}%",
            f"{len(df[(df['Sex'] == 'female') & (df['Survived'] == 1)])/len(df[df['Sex'] == 'female'])*100:.1f}%",
            f"{len(df[(df['Pclass'] == 1) & (df['Survived'] == 1)])/len(df[df['Pclass'] == 1])*100:.1f}%",
            f"{len(df[(df['Pclass'] == 2) & (df['Survived'] == 1)])/len(df[df['Pclass'] == 2])*100:.1f}%",
            f"{len(df[(df['Pclass'] == 3) & (df['Survived'] == 1)])/len(df[df['Pclass'] == 3])*100:.1f}%"
        ]
    })
...
# Подвал
st.markdown("---")
st.markdown("Дашборд создан для анализа данных пассажиров Титаника")
